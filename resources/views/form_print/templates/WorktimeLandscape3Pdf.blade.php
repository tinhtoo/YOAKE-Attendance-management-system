...
<html lang="ja">
    ...
    @isset($wtLandscape3Datas)
    <!-- 対象データがない場合 -->
    @if (count($wtLandscape3Datas) < 1)
    <body>
        <!-- ページ番号作成 -->
        ...
        <div class="record">
            ...
            <span>:</span>
            <span>{{ date('Y/m/d', strtotime($str_date)) }}</span>
            <span>～</span>
            <span>{{ date('Y/m/d', strtotime($end_date)) }}</span><br>
            @else
            <span>対象月度</span>
            <span>:</span>
            <span>
    ...
    <!-- 社員コード毎で帳票出力 -->
    @foreach ($wtLandscape3Datas->unique('EMP_CD') as $reportData)
    @php
        $sunday_hh = 0;
        $sunday_mi = 0;
        $sunday_mi_d60 = 0;
    ...
    <body>
        ...
        <div class="record">
            <span style="margin-left:20px;">社員</span>
            <span>:</span>
            <span style="margin-left:5px;">{{ $reportData['EMP_CD'] }}</span>
            <span style="margin-left:30px;">{{ $reportData['EMP_NAME'] }}</span>
            @if ($reportData['EMP2_CD'])
            <span style="margin-left:20px;">({{ $reportData['EMP2_CD'] }})</span>
            @endif
            <span style="margin-left:80px; padding-left: 110px;">部門</span>
            <span>:</span>
            <span style="margin-left:5px;">{{ $reportData['DEPT_CD'] }}</span>
            <span style="margin-left:5px;">{{ $reportData['DEPT_NAME'] }}</span>
            <span style="margin-left:120px; padding-left: 20px;">所属先</span>
            <span>:</span>
            <span style="margin-left:5px;">{{ $reportData['COMPANY_CD'] }}</span>
            <span style="margin-left:5px;">{{ $reportData['COMPANY_NAME']}}</span>
        </div>
        <table class="workTable">
            <thead>
                <tr>
            ...
            @endphp
            <tbody>
            @foreach ($emp_wt_list as $i => $wt_data)
            @php
                $back_color = '';
            ...
            @endphp
                <tr>
                    <td class="{{ $back_color }}" style="padding-left: 4px; text-align: center;">
                        {{ date('n/d', strtotime($wt_data['CALD_DATE'])) }}
                        ({{ config('consts.weeks')[date('w', strtotime($wt_data['CALD_DATE']))] }})
                    </td> <!-- 日付 -->
                    <td class="{{ $back_color }}" style="text-align: center;">
                        {{ $wt_data['HOLDAY_MARK'] }}
                    </td> <!-- 休 -->
                    <td class="{{ $back_color }}" style="text-align: left;">
                        {{ $wt_data['WORKPTN_NAME'] }}
                    </td> <!-- 勤務体系 -->
                    <td class="{{ $back_color }}" style="text-align: center;">{{ $wt_data['REASON_NAME'] }}
                    </td> <!-- 届け -->
                    <td class="{{ $back_color }}">
                        {{ $wt_data['OFC_TIME'] == '0:00' ? '' : $wt_data['OFC_TIME'] }}
                    </td> <!-- 出勤 -->
                    <td class="{{ $back_color }}" style="text-align: left;">
                        {{ $wt_data['LEV_TIME'] == '0:00' ? '' : $wt_data['LEV_TIME'] }}
                    </td> <!-- 退出 -->
                    <td class="{{ $back_color }}" style="text-align: left;">
                        {{ $wt_data['TLO_MARK'] }}
                    </td> <!--　遅早残 -->
                    <td class="{{ $back_color }}">
                        {{ $wt_data['OUT1_TIME'] == '0:00' ? '' : $wt_data['OUT1_TIME'] }}
                    </td> <!--　外出１ -->
                    <td class="{{ $back_color }}">
                        {{ $wt_data['IN1_TIME'] == '0:00' ? '' : $wt_data['IN1_TIME']}}
                    </td> <!--　再入１ -->
                    <td class="{{ $back_color }}">
                        {{ $total_time == '0:00' ? '' : $total_time }}
                    </td> <!-- 実働 -->
                    <td class="{{ $back_color }}">
                        {{ $def_time == '0:00' ? '' : $def_time }}
                    </td> <!-- 所定 -->
                    <td class="{{ $back_color }}">
                        {{ $ext1_time == '0:00' ? '' : $ext1_time }}
                    </td> <!-- 深割 -->
                    <td class="{{ $back_color }}">
                        {{ $normal_time == '0:00' ? '' : $normal_time }}
                    </td> <!-- 普残 -->
                    <td class="{{ $back_color }}">
                        {{ $late_night_ovtm_time == '0:00' ? '' : $late_night_ovtm_time }}
                    </td> <!-- 深残 -->
                    <td class="{{ $back_color }}">
                        {{ $hol_in_time == '0:00' ? '' : $hol_in_time }}
                    </td> <!-- 法休 -->
                    <td class="{{ $back_color }}">
                        {{ $hol_in_ovtm_time == '0:00' ? '' : $hol_in_ovtm_time }}
                    </td> <!-- 法深残 -->
                    <td class="{{ $back_color }}">
                        {{ $hol_out_time == '0:00' ? '' : $hol_out_time }}
                    </td> <!-- 休出 -->
                    <td class="{{ $back_color }}">
                        {{ $hol_out_ovtm_time == '0:00' ? '' : $hol_out_ovtm_time }}
                    </td> <!-- 休残 -->
                    <td class="{{ $back_color }}">
                        {{ $hol_out_late_night_ovtm_time == '0:00' ? '' : $hol_out_late_night_ovtm_time }}
                    </td> <!-- 休深残 -->
                    <td class="{{ $back_color }}">
                        {{ $tard_leave_time == '0:00' ? '' : $tard_leave_time }}
                    </td> <!-- 遅早 -->
                    <td class="{{ $back_color }}">
                        {{ $out_time == '0:00' ? '' : $out_time }}
                    </td> <!-- 外出 -->
                    <td class="{{ $back_color }}" style="text-align: left;">
                        {{ $wt_data['REMARK'] }}
                    </td> <!-- 備考 -->
            ...
            @endphp
                <tr class="sum-data">
                    ...
                    <td></td>
                    <td></td>
                    <td>{{ $sum_total == '0:00' ? '' : $sum_total }}</td> <!-- 実働計 -->
                    <td>{{ $sum_def == '0:00' ? '' : $sum_def }}</td> <!-- 出勤計 -->
                    <td>{{ $sum_ext1 == '0:00' ? '' : $sum_ext1 }}</td> <!-- 深割計 -->
                    <td>{{ $sum_normal == '0:00' ? '' : $sum_normal }}</td> <!-- 普残計 -->
                    <td>{{ $sum_late_night_ovtm == '0:00' ? '' : $sum_late_night_ovtm }}</td> <!-- 深残計 -->
                    <td>{{ $sum_hol_in == '0:00' ? '' : $sum_hol_in }}</td> <!-- 法休計 -->
                    <td>{{ $sum_hol_in_ovtm == '0:00' ? '' : $sum_hol_in_ovtm }}</td> <!-- 法深残計 -->
                    <td>{{ $sum_hol_out == '0:00' ? '' : $sum_hol_out }}</td> <!-- 休出計 -->
                    <td>{{ $sum_hol_out_ovtm == '0:00' ? '' : $sum_hol_out_ovtm }}</td> <!-- 休残計 -->
                    <td>
                        {{ $sum_hol_out_late_night_ovtm == '0:00' ? '' : $sum_hol_out_late_night_ovtm }}
                    </td><!-- 休深計 -->
                    <td>{{ $sum_tard_leave_time == '0:00' ? '' : $sum_tard_leave_time }}</td> <!-- 遅早計 -->
                    <td>{{ $sum_out_time == '0:00' ? '' : $sum_out_time }}</td> <!-- 外出計 -->
                    <td style="text-align: left;">日曜 : {{ $sum_total_sunday }}</td> <!-- 日曜出勤時間 -->
                </tr>
            </tbody>
        </table>
        <table class="footerTable">
            <tbody class="footer">
                ...
                <tr>
                    <td>
                        {{ $wt_data['SUM_WORKDAY_CNT'] == '0.0' ? '' : $wt_data['SUM_WORKDAY_CNT'] }}
                    </td><!-- 出勤日数 -->
                    <td>
                        {{ $sum_def == '0:00' ? '' : $sum_def }}
                    </td><!-- 所定時間 -->
                    <td>
                        {{ $wt_data['SUM_ABCWORK_CNT'] == '0.0' ? '' : $wt_data['SUM_ABCWORK_CNT'] }}
                    </td><!-- 欠勤日数 -->
                    <td>
                        {{ $wt_data['SUM_PADHOL_CNT'] == '0.0' ? '' : $wt_data['SUM_PADHOL_CNT'] }}
                    </td><!-- 有休日数 -->
                    <td>
                        {{ $wt_data['SUM_SPCHOL_CNT'] == '0.0' ? '' : $wt_data['SUM_SPCHOL_CNT'] }}
                    </td><!-- 特休日数 -->
                    <td>
                        {{ $sum_ext1_day == '0' ? '' : $sum_ext1_day }}
                    </td><!-- 深夜割増日数 -->
                    <td>
                        {{ $sum_ext1 == '0:00' ? '' : $sum_ext1 }}
                    </td><!-- 深夜割増 -->
                    <td>
                        {{ $sum_normal == '0:00' ? '' : $sum_normal }}
                    </td><!-- 普通残業 -->
                    <td>
                        {{ $sum_late_night_ovtm == '0:00' ? '' : $sum_late_night_ovtm }}
                    </td><!-- 深夜残業 -->
                    <td>
                        {{ $sum_hol_in == '0:00' ? '' : $sum_hol_in }}
                    </td><!-- 法休 -->
                    <td>
                        {{ $sum_hol_in_ovtm == '0:00' ? '' : $sum_hol_in_ovtm }}
                    </td><!-- 法休深夜 -->
                    <td>
                        {{ $sum_hol_out == '0:00' ? '' : $sum_hol_out }}
                    </td><!-- 休日出勤 -->
                    <td>
                        {{ $sum_hol_out_ovtm == '0:00' ? '' : $sum_hol_out_ovtm }}
                    </td><!-- 休日残業 -->
                    <td>
                        {{ $sum_hol_out_late_night_ovtm == '0:00' ? '' : $sum_hol_out_late_night_ovtm }}
                    </td><!-- 休日深夜 -->
                    <td>
                        {{ $sum_tard_leave_time == '0:00' ? '' : $sum_tard_leave_time }}
                    </td><!-- 遅早 -->
                    <td>
                        {{ $sum_out_time == '0:00' ? '' : $sum_out_time }}
                    </td><!-- 外出 -->
                </tr>
            </tbody>
        </table>
