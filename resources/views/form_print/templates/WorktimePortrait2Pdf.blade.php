...
<html lang="ja">
    ...
    @isset($wtPortrait2Datas)
    <!-- 該当データがない場合 -->
    @if (count($wtPortrait2Datas) < 1)
    <body>
        <!-- ページ番号作成 -->
        ...
        <div class="date">
            ...
            <span style="margin-right:50px;">{{ $now_date }}</span>
        </div>
        ...
        <table class="workTable" style="width: 100%;">
            ...
            @endforeach
                ...
                <tr class="record-sum">
                    ...
                    <td>社員計</td>
                    <td>
                        @if ($emp_worktime_list->whereNotNull('DEF_WORK_TIME_HH')->count())
                        {{ $sum_def_hh . ':' . $sum_def_mi }}
                        @endif
                    </td>
                    <td>
                        @if ($emp_worktime_list->whereNotNull('HOL_WORK_TIME_HH')->count())
                        {{ $sum_hol_hh . ':' . $sum_hol_mi }}
                        @endif
                    </td>
                    <td>
                        @if ($emp_worktime_list->whereNotNull('HOL_OUT_WORK_TIME_HH')->count())
