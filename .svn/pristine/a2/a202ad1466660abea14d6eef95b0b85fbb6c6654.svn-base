...
class MT06OverTmLmtEditorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($CalendarCdKey != null) {
            ...
            $calendar_ptn = MT02CalendarPtn::where('CALENDAR_CD', $CalendarCdKey)->first();
            if ($calendar_ptn === null) {
                return parent::viewErrorWithMenu([
                    'view_path' => '/master/MT06OverTmLmtEditor',
                    'view_name' => '残業上限情報入力',
                    'message' => '2000',
                ]);
            }
            $enabled02 = $calendar_ptn['CALENDAR_CLS_CD'];
