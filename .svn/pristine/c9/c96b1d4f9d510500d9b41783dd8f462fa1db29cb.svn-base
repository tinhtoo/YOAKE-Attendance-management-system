...
<html lang="ja">
    ...
    <!-- 該当データ有 -->
    <!-- 日付ごとでグループ化 -->
    @foreach ($dailyReportDatas->groupBy('CALD_DATE_FOR_GROUP') as $report_date)
        <!-- 同じ日付の部門ごとで表示 -->
        @php
            ...
            <table class="workTable">
                <thead>
                    ...
                    <tr>
                        <th style="border: none; height:4px; text-align:right;">対象日 : </th>
                        <th colspan="12" style="border: none; height:4px; text-align:left;">
                            {{ date('Y/m/d', strtotime($dept->CALD_DATE)) }}
                            ({{ config('consts.weeks')[date('w', strtotime($dept->CALD_DATE))] }})
                        </th>
                    </tr>
                    <tr>
                        <th style="border: none; height:4px; text-align:right; padding-bottom:9px;">部門 : </th>
                        <th colspan="12" style="border: none; height:4px; text-align:left; padding-bottom:9px;">
                            {{ $dept->DEPT_CD }}&nbsp;&nbsp;&nbsp;{{ $dept->DEPT_NAME }}
                        </th>
                    </tr>
                    <tr style="width: 100%;">
                        <th style="padding-left: 5px; text-align: left;">社員<br>&nbsp;&nbsp;</th>
                        ...
                        <th style="width: 75px;">
                            {{ $dept->WORK_DESC_NAME_102 }}<br>
                            {{ $dept->WORK_DESC_NAME_103 }}
                        </th>
                        <th style="width: 75px;">
                            {{ $dept->WORK_DESC_NAME_104 }}<br>&nbsp;&nbsp;
                            {{ $dept->WORK_DESC_NAME_105 }}
                        </th>
                        <th style="width: 75px;">
                            {{ $dept->WORK_DESC_NAME_200 }}<br>&nbsp;&nbsp;
                            {{ $dept->WORK_DESC_NAME_201 }}
                        </th>
                        <th style="width: 75px; padding-right: 5px;">
                            {{ $dept->WORK_DESC_NAME_202 }}<br>&nbsp;&nbsp;
                        </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @endphp
                    @foreach ($emp_worktime_list as $key => $emp_wt_data)
                    <tr style="width: 100%;">
                        <td style="padding-left: 5px; text-align: left;">
                            {{ $emp_wt_data->EMP_CD }}<br>&nbsp;&nbsp;
                        </td>
                        <td style="text-align: left;">
                            {{ $emp_wt_data->EMP_NAME }}<br>&nbsp;&nbsp;
                        </td>
                        <td style="text-align: left;">
                            {{ $emp_wt_data->WORKPTN_NAME }}<br>{{ $emp_wt_data->REASON_NAME }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OFC_TIME }}<br>&nbsp;&nbsp;{{ $emp_wt_data->LEV_TIME }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OUT1_TIME }}<br>&nbsp;&nbsp;{{ $emp_wt_data->IN1_TIME }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OUT2_TIME }}<br>&nbsp;&nbsp;{{ $emp_wt_data->IN2_TIME }}
                        </td>
                        <td>
                            {{ $emp_wt_data->WORK_TIME_HH . ':' . substr('00' . $emp_wt_data->WORK_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->TARD_TIME_HH . ':' . substr('00' . $emp_wt_data->TARD_TIME_MI, -2) }}
                        </td>
                        <td>
                            {{ $emp_wt_data->LEAVE_TIME_HH . ':' . substr('00' . $emp_wt_data->LEAVE_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->OUT_TIME_HH . ':' . substr('00' . $emp_wt_data->OUT_TIME_MI, -2) }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OVTM1_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM1_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->OVTM2_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM2_TIME_MI, -2) }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OVTM3_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM3_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->OVTM4_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM4_TIME_MI, -2) }}
                        </td>
                        <td>
                            {{ $emp_wt_data->OVTM5_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM5_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->OVTM6_TIME_HH . ':' . substr('00' . $emp_wt_data->OVTM6_TIME_MI, -2) }}
                        </td>
                        <td>
                            {{ $emp_wt_data->EXT1_TIME_HH . ':' . substr('00' . $emp_wt_data->EXT1_TIME_MI, -2) }}
                        <br>
                            {{ $emp_wt_data->EXT2_TIME_HH . ':' . substr('00' . $emp_wt_data->EXT2_TIME_MI, -2) }}
                        </td>
                        <td style="padding-right: 5px;">
                            {{ $emp_wt_data->EXT3_TIME_HH . ':' . substr('00' . $emp_wt_data->EXT3_TIME_MI, -2) }}
                        <br>&nbsp;&nbsp;
                        </td>
                    </tr>   
                    @endforeach
                    ...
                    <tr class="record-sum">
                        ...
                        <td></td>
                        <td>部門計</td>
                        <td>{{ $sum_work_time }}<br>&nbsp;{{ $sum_tard_time }}</td>
                        <td>{{ $sum_leave_time }}<br>&nbsp;{{ $sum_out_time }}</td>
                        <td>{{ $sum_ovtm1_time }}<br>&nbsp;{{ $sum_ovtm2_time }}</td>
                        <td>{{ $sum_ovtm3_time }}<br>&nbsp;{{ $sum_ovtm4_time }}</td>
                        <td>{{ $sum_ovtm5_time }}<br>&nbsp;{{ $sum_ovtm6_time }}</td>
                        <td>{{ $sum_ext1_time }}<br>&nbsp;{{ $sum_ext2_time }}</td>
                        <td style="padding-right: 5px;">{{ $sum_ext3_time }}<br>&nbsp;</td>
                    </tr>
                </tbody>
            </table>
            <!-- フッター -->
