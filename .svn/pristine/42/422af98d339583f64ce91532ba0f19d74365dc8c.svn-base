...
class MT05WorkPtnEditorController extends Controller
{
    ...
    {
        $data_work_ptn = new MT05Workptn();
        if ($id != null) {
            $data_work_ptn = $this->mt05_workptn->getWorkPtnWithPrimaryKey($id);
            if ($data_work_ptn === null) {
                return parent::viewErrorWithMenu([
                    'view_path' => '/master/MT05WorkPtnReference',
                    'view_name' => '勤務体系情報入力',
                    'message' => '2000',
                ]);
            }
        ...
        // 勤怠項目
        $data_work_desc_exp02 = $this->mt05_workptn->getWorkDescExcepting02();
        // 割増
        $data_extra_pay_cls_cd = '02';
        $data_extra_pay = $this->mt05_workptn->getWithWorkDesc($data_extra_pay_cls_cd);
        // 職務種別
        $cls_detail_cd = '08';
        $cls_details = $this->mt05_workptn->getClsDetail($cls_detail_cd);

        return parent::viewWithMenu('master.MT05WorkPtnEditor', compact(
            'data_work_ptn',
            'data_work_desc_exp02',
            'data_extra_pay',
            'cls_details'
        ));
    }
